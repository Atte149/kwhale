...
DATA_ROOT="${DATA_ROOT:-/files/kwhale/data}"
MUSIC_SRC="${1:-}"
...
if [ ! -f mcp/.env ]; then
    echo "DATABASE_URL=postgresql://melorise:melorise_secret@postgres:5432/melorise" > mcp/.env
    echo "[+] Created mcp/.env"
fi

echo ""
echo "=== Next steps ==="
echo "1. Edit .env and api/.env with your passwords and API keys"
echo "2. docker compose build"
echo "3. docker compose up -d"
echo "4. Open http://localhost:19000/docs to verify"
echo "5. Create Navidrome admin at http://localhost:4535 (first launch)"
echo "6. POST http://localhost:19000/api/auth/login to get your token"
echo "7. POST http://localhost:19000/api/vibe/index-all to start indexing"
